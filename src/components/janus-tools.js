          ...
          janus.attach({
            ...
            onmessage: function (msg, jsep) {
              ...
              if (event) {
                ...
                if (event == "talking") {
                    self.participantChangeStatus(msg["id"], 'talking')
                }
                if (event == "stopped-talking") {
                    self.participantChangeStatus(msg["id"], 'stopped-talking')
                }
                if (event === "joined") {
                  ...
                  if (msg["participants"]) {
                    ...
                    for (var f in list) {
                      ...
                      if(muted){
                        self.participantChangeStatus(list[f]["id"], 'stopped-talking')
                      }
                      self.addParticipant(list[f]["id"], list[f]["display"]);
                      /*
                                                    if ($('#rp' + id).length === 0) {
  ...
  if (participants.length > 0) {
    for (let i = 0; i < participants.length; i++) {
      ...
      if (participants[i].id == participantId) {
        ...
        self.setState(newState, () => console.log(self.state.participants));
      }
    }
  }
}
...
export function toggleMute() {
  ...
  //if(self.state.muted){
    self.participantChangeStatus(this.state.myId, "stopped-talking")
  //}
